...
サーバーでは，`vim`を使ってプログラムファイルを作る．`touch test.py`と適当にファイルを造ったら，`vi test.py`としてターミナル上で`vim`を開く．`i`と打つことで，編集ができる．やめるときは，`escape`を押し，閉じるときは`:wq`とする．サーバーを閉じるときは，`exit`とする．
